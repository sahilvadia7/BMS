...
public class EducationLoanServiceImpl implements EducationLoanService {
        ...
        public LoanEvaluationResponse evaluateLoan(Long loanId) {
                ...
                // Extract data
                BigDecimal requestedAmount = loan.getRequestedAmount() != null ? loan.getRequestedAmount()
                                : BigDecimal.ZERO;
                BigDecimal totalCourseCost = edu.getTotalCourseCost() != null ? edu.getTotalCourseCost()
                                : BigDecimal.ZERO;
                BigDecimal coApplicantIncome = edu.getCoApplicantAnnualIncome() != null
                                ? edu.getCoApplicantAnnualIncome()
                                : BigDecimal.ZERO;
                ...
                BigDecimal ltv = totalCourseCost.compareTo(BigDecimal.ZERO) > 0
                                ? requestedAmount.divide(totalCourseCost, 4, RoundingMode.HALF_UP)
                ...
                if (coApplicantIncome.compareTo(BigDecimal.valueOf(300000)) >= 0)
                        totalPoints += 20;
                else if (coApplicantIncome.compareTo(BigDecimal.valueOf(100000)) >= 0)
                        totalPoints += 10;
                ...
                // 5. Tenure check
                if (tenure >= rate.getMinTenure() && tenure <= rate.getMaxTenure())
                        totalPoints += 10;

                // 6. Moratorium
                if (moratorium <= 6)
                        totalPoints += 10;
                else if (moratorium <= 12)
                        totalPoints += 5;
                ...
                if (courseTotal.compareTo(totalCourseCost) == 0)
                        totalPoints += 10;

                // 8. Employment type
                if (loan.getEmploymentType() == EmploymentType.STUDENT)
                        totalPoints += 5;
